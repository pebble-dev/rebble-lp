...
def upload_pbl(file, locale):
    filename = f"{id_generator.generate()}-{locale}.pbl"
    s3_filename = f"{config['S3_PATH']}{filename}"
    s3 = _client_for_endpoint(s3_endpoint)
    if isinstance(file, str):
        s3.upload_file(file, config['S3_BUCKET'], s3_filename, ExtraArgs = { 'ContentType': 'application/octet-stream' })
    else:
        file.seek(0)
        s3.upload_fileobj(file, config['S3_BUCKET'], s3_filename, ExtraArgs = { 'ContentType': 'application/octet-stream' })
    return filename

def download_pbl(filename, file):
    s3_filename = f"{config['S3_PATH']}{filename}"
    s3 = _client_for_endpoint(s3_endpoint)
    if isinstance(file, str):
        s3.download_file(config['S3_BUCKET'], s3_filename, file)
    else:
        s3.download_fileobj(config['S3_BUCKET'], s3_filename, file)
